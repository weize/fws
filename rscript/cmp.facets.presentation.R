...
outfile <- sprintf("figure/presentation.cmp-facet-%s-%s-%s.0.eps", feedbackSrc, expModel, metricName)
...
all <- list(annotator, gmi, qd, lda)
names <- c("annotator", "QF-I", "QDM", "LDA")
dataNum <- length(all) 
colors <- rainbow(dataNum+1)
